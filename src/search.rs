...
pub async fn search_source(
    ...
    match search_source_once(client, &source, query, limit, api_key).await {
        Ok(results) => Ok(results),
        Err(error) if is_transient(&error) => {
            tokio::time::sleep(TRANSIENT_RETRY_DELAY).await;
            search_source_once(client, &source, query, limit, api_key).await
        }
        Err(error) => Err(error),
    }
}
...
async fn search_source_once(
    ...
    let endpoint = torznab::build_url(
        &source.endpoint,
    ...
    let mut results =
        torznab::parse_search_response(&body).with_context(|| format!("{}", source.endpoint))?;
...
pub async fn search_all(
    ...
    for batch in sources.chunks(fanout) {
        let searches =
            stream::iter(
                batch.iter().cloned().map(|source| {
                    let client = client.clone();
                    let query = query.to_string();
                    async move {
                        search_source(&client, source, &query, per_source_limit, api_key).await
                    }
                }),
            )
